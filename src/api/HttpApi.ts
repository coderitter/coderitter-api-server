    ...
    async handlePostOnlyRequest(
        ...
        try {
            remoteMethodCall = <RemoteMethodCall>(
                fromJsonObj(data, { instantiator: this.instantiator })
            )
        }
        catch (e) {
            l.error('Could not parse the JSON containing the RemoteMethodCall')
            result = Result.remoteError(
                'Could not parse the JSON containing the RemoteMethodCall'
            )
        }
        ...
        try {
            ...
            let errorResult = Result.remoteError(
                'Could not stringify result to JSON'
            )
